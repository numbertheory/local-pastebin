...
def index():
    ...
    recent_pastes = [
        {
            ...
            "content": x.content,
            "version": x.version,
            "created_at": set_to_timezone(x.created_at),
        }
        for x in pagination.items
    ]
    for i in range(0, len(recent_pastes)):
        pastes = (
            Paste.query.filter_by(id=recent_pastes[i]["id"])
            .order_by(Paste.version.desc())
            .all()
        )
        history = [
            {"version": p.version, "created_at": set_to_timezone(p.created_at)}
            for p in pastes
        ]
    ...
    return render_template(
        "index.html", recent_pastes=recent_pastes, pagination=pagination
    )
...
def api_get_paste(paste_id):
    if request.method == "GET":
        paste = (
            Paste.query.filter_by(id=paste_id)
            .order_by(Paste.version.desc())
            .first_or_404()
        )
